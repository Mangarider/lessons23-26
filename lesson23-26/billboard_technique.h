...
bool BillboardTechnique::Init()
{
    ...
    if (!AddShader(GL_VERTEX_SHADER, pVS4)) {
        return false;
    }

    if (!AddShader(GL_GEOMETRY_SHADER, pGS4)) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, pFS4)) {
        return false;
    }
