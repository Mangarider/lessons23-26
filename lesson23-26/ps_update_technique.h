...
bool PSUpdateTechnique::Init()
{
    ...
    ;
    if (!AddShader(GL_VERTEX_SHADER, pVS5)) {
        return false;
    }

    if (!AddShader(GL_GEOMETRY_SHADER, pGS5)) {
        return false;
    }

    if (!AddShader(GL_FRAGMENT_SHADER, pFS5)) {
        return false;
    }
